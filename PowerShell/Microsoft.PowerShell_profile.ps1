...
# open nvim in same directory
function nvimhere {
    nvim (Get-Location)
}

#open IDE multi pane powershell for development when ide command is entered. there should be 3 powershells in which 1st powershell should take full width and 2nd and 3rd powershell should be in bottom split with equal width.
function ide {
# Get the current directory
  $currentDir = Get-Location

# Get the PID (process ID) of the most recently active Windows Terminal window, if any.
    $existingPid = (Get-Process -ErrorAction Ignore | Where-Object { $_.Name -eq "WindowsTerminal" } | Select-Object -Last 1).Id

# Open PowerShell in the same window with horizontal and vertical splits
# -w 0 targets the most recently active window, if any.
    wt.exe -w 0 -d $currentDir pwsh `; split-pane -H -s 0.3 -d $currentDir pwsh `; split-pane -V -d $currentDir pwsh

# If a preexisting window was targeted, activate it explicitly.
    if ($existingPid) {
      try {
        (New-Object -ComObject WScript.Shell).AppActivate($existingPid)
      } catch {
        Write-Host "Failed to activate the existing window with PID $existingPid"
      }
    }
}
...
# when the powerhsell is opened run clean command 
function cleanLogs {
  [System.Console]::Clear()
}
#run the cleanLogs function when the powershell is opened
cleanLogs
